...
final holdingsMarketDataProvider = FutureProvider<List<EnrichedHolding>>((
  ref,
) async {
  final holdingsWithInstruments = await ref.watch(
  ...
  for (final h in holdingsWithInstruments) {
    ...
    final valueInNative = Money(
      minor: (quantity * Decimal.fromInt(effectivePriceMinor))
          .toBigInt()
          .toInt(),
    ...
    final valueInBase = Money.fromDecimal(
      valueInNative.decimal * rate,
      baseCurrency,
    );
